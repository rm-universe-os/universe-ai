"""
Universe AI — English terminal bridge to the Ollama brain.

Chat with the same agent that lives on the desktop, straight from a terminal:

    python3 brain/ollama-chat.py "check disk space"
    python3 brain/ollama-chat.py            # interactive REPL

Model / URL can be overridden with --model / --url or UAI_MODEL / UAI_OLLAMA_URL.
Tools (run_command with approval, web_search, fetch_url, edit_file) mirror main.js.
"""
import argparse
import json
import os
...
def classify(cmd: str):
    """Return ('run'|'ask'|'refuse', reason). Conservative: anything the
    word-splitter cannot fully see is sent to approval."""
    import re
    if re.search(r"[\n\r]", cmd):
